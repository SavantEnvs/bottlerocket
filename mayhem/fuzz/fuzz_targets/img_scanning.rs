...
// gptman 1.1.4 (the version core-kit pins) computes `len / sector_size - 1` for the
// backup-header seek in GPT::read_from; for an image shorter than one sector that
// underflows (a panic under this build's overflow checks, a wrapped seek in release).
// GPT::find_from tries sector size 512 and THEN 4096, so every image shorter than 4096
// bytes trips it — which is every early libFuzzer input, and it starved three cloud runs
// to 0 edges (the previous `< 512` guard only covered the first attempt). A real block
// device is never shorter than one sector, so zero-pad short inputs up to one 4096-byte
// sector instead of returning: the fuzzer keeps exploring header parsing with SHORT,
// fast-to-mutate inputs, and the length band 512..4095 is no longer a dead crash.
// Reproducer + upstream fix: mayhem/img-scanning/known-findings/gptman-short-image-seek-underflow/
const MIN_IMAGE_LEN: usize = 4096;

fuzz_target!(|data: &[u8]| {
    let image: std::borrow::Cow<[u8]> = if data.len() < MIN_IMAGE_LEN {
        let mut padded = data.to_vec();
        padded.resize(MIN_IMAGE_LEN, 0);
    ...
    let _ = find_device_type(&mut cursor);
});
